...
const Header: React.FC<HeaderProps> = ({ onMenuClick, darkMode, user }) => {
  ...
  const toggleHeaderVisibility = () => {
    setIsHeaderVisible((prev) => {
      const next = !prev;
      localStorage.setItem('mirov_header_visible', JSON.stringify(next));
      window.dispatchEvent(new CustomEvent('headerVisibilityChange', { detail: next }));
      return next;
    });
  };
  ...
  return (
    <div className="relative z-30 flex-shrink-0">
      ...
      {/* Floating Toggle Pull-Tab outside Header */}
